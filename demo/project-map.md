...
```
   Источники знаний
   ┌───────────────────────────┐    ┌───────────────────────────┐
   │ Сессия харнеса (opencode) │    │  .md файлы (learnings/)   │
...
**Ключевое:** извлечение делает агент харнеса (он уже LLM с полным контекстом
...
> Извлечение из сессий делает **агент харнеса** (opencode-скилл `/curator-save`):
...
```
ШАГ 1: Появление
   Источник: агент харнеса передаёт candidates (сессия)
...
ШАГ 7: Авто-забывание (автономно, в worker.py)
   Файл: worker.py (авто-decay через RetrievalFeedback)
   unused > 30 дней → verified → hypothesis
   unused > 90 дней → hypothesis → deprecated
...
- [ ] **3.** Я могу показать как факт попадает в систему: агент харнеса извлекает → candidates → MCP → gatekeeper → backend
