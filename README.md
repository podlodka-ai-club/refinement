...
- **Меняет поведение агента сам** — инсталлер вписывает правила памяти в
  глобальный AGENTS.md / CLAUDE.md: база в контексте каждой сессии, агент
  сверяется с ней до задач и сам предлагает сохранить выученное
  (плагин session.idle напомнит, если забыл)
...
- **Авто-триггер извлечения**: полный автоном без human-in-the-loop
  (сейчас — напоминание плагином и подтверждение человеком)
